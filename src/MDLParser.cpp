...
static std::unordered_multimap<int, int> m;
...
{
    std::pair<std::unordered_multimap<int, int>::iterator, std::unordered_multimap<int, int>::iterator> result;
    std::unordered_multimap<int, int>::iterator it;
    for (unsigned int i = 0; i < graph.connections.size(); ++i) {
        if (graph.connections[i].size() > 1) {
            ...
            if (roadRegistry->getRoad(i) != nullptr)
                ...
                for (it = result.first; it != result.second; ++it) {
                        if (roadRegistry->getRoad(it->second) != nullptr)
                        roadRegistry->connectRoads(x, it->second);
                }
            it = result.first;  
        ...
        if (graph.back_connections[i].size() > 1) {
